...
TouchDrvInterface *make_touch() {
    if (hw_is_v2()) {
        ...
        if (!t->begin(Wire, CST816_SLAVE_ADDRESS, IIC_SDA, IIC_SCL))
            return nullptr;
        t->disableAutoSleep();   // keep CST816/CST820 responsive to polling
        return t;
    }
    TouchDrvFT6X36 *t = new TouchDrvFT6X36();
    if (!t->begin(Wire, FT6X36_SLAVE_ADDRESS, IIC_SDA, IIC_SCL))
        return nullptr;
    return t;
}
